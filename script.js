...
let db_races = []; /**
 * Lógica principal para F1 Calendario 2026
 * DATOS OFICIALES: FORMULA1.COM + TV INFO + OPENF1 (ORDENACIÓN CORREGIDA)
 */
...
async function loadResultsForRace(round) {
    ...
    try {
        ...
        if (results && results.length > 0) {
            // --- LÓGICA DE ORDENACIÓN REFORZADA ---
            // Ponemos a los que no tienen posición (null) al final dándoles el valor 999
            results.sort((a, b) => (a.position || 999) - (b.position || 999));
            
            // Buscamos el tiempo del ganador (el que realmente quedó 1º)
            ...
            race.results = results.map(r => {
                ...
                // Si tiene posición 1, mostrar su tiempo total
                if (r.position === 1 && r.time) {
                    timeLabel = formatTime(r.time);
                } 
                // Si tiene tiempo y no es el ganador, calcular GAP
                else if (r.time && winnerTime) {
                    const gap = ((r.time - winnerTime) / 1000).toFixed(3);
                    timeLabel = `+${gap}s`;
                } 
                // Si es nulo o DNF, traducir estado
                else {
                    timeLabel = translateStatus(r.status);
                }

                return {
                    pos: r.position || "NC", // NC = No Clasificado
