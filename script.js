    ...
    try {
        ...
        if (raceData && raceData.Results && raceData.Results.length > 0) {
            race.results = raceData.Results.map(r => {
                ...
                // 2. Si el status indica que ha perdido vueltas, lo ponemos en minúsculas (+1 lap, +2 laps)
                else if (r.status.includes("Lap")) {
                    displayTime = r.status.toLowerCase();
                }
...
async function loadResultsForRace(round) {
    ...
    try {
        ...
        if (raceData && raceData.Results && raceData.Results.length > 0) {
            ...
            // Calculamos las vueltas del líder (el ganador siempre está en la posición 0 del array)
            const winnerLaps = parseInt(raceData.Results[0].laps);

            race.results = raceData.Results.map(r => {
                let displayTime = r.status; 
                
                // Calculamos cuántas vueltas dio este piloto y cuántas le sacó el líder
                const driverLaps = parseInt(r.laps);
                ...
                // 2. Si el status es "Lapped" o contiene "Lap", forzamos el cálculo de vueltas
                else if (r.status.toLowerCase().includes("lap")) {
